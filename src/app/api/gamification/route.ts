...
export async function GET() {
  try {
    ...
    // 1. Fetch completed habit logs with habit info
    ...
    // 5. Fetch defeated boss battles
    const defeatedBosses = await db.bossBattle.findMany({
      where: { completed: true },
    });

    // 6. Calculate total and today XP
    let totalXp = 0;
    let todayXp = 0;
    ...
    for (const boss of defeatedBosses) {
      ...
      intellectPoints += 8;
      agilityPoints += 8;
      if (boss.deadlineDate === todayStr) {
        todayXp += XP_REWARDS.BOSS_BATTLE_VICTORY;
      }
    }

    // 5. Calculate streak (consecutive active days)
    const datesWithActivity = new Set<string>();
    habitLogs.forEach((l) => datesWithActivity.add(l.date));
    completedTasks.forEach((t) => datesWithActivity.add(t.date));
    ...
    return NextResponse.json({ error: "Error al calcular gamificación" }, { status: 500 });
  }
}
